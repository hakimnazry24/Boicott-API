...
app.post("/retailer", (req, res) => {
    ...
    db.run(sql, [name, country], (err) => {
        if (err) {
            res.status(500).send({error: `Cannot create ${name}, ${country} ${err}`});
            return;
        }
        res.status(200).send({message: `Successfully  creating new retailer: ${JSON.stringify(req.body)}`})
    })
});

app.post("/product", (req, res) => {
    const { name, serial_number, retailer_id, company_id } = req.body;
    const sql = "INSERT INTO Product(name, serial_number, retailer_id, company_id) VALUES(?, ?, ?, ?)";
    db.run(sql, [name, serial_number, retailer_id, company_id], (err) => {
        if (err) {
            res.status(500).send({error: `Cannot create ${JSON.stringify(req.body)}`});
            return;
        }
        res.status(200).send({message: `Successfully creating new product: ${JSON.stringify(req.body)} `});
    })
});

app.post("/company", (req, res) => {
    const { name, status, status_reason, evidence_link, image, about, more_info  } = req.body;
    const sql = "INSERT INTO Retailer(name, country) VALUES(?, ?)";
    db.run(sql, [name, status, status_reason, evidence_link, image, about, more_info], (err) => {
        if (err) {
            res.status(500).send({error: `Cannot create ${JSON.stringify(req.body)}`});
            return;
        }
        res.status(200).send({message: `Successfully creating new company: ${JSON.stringify(req.body)}`})
    })
});
...
app.delete("/retailer/:id", (req, res) => {
    ...
    db.get(selectSql, [params], (err, row) => {
        if (err) {
            ...
            res.status(404).send({error: `Retailer with id ${params} does not exists`});
        } else {
            db.run(deleteSql, [params], (err) => {
                ...
                res.status(200).send({message: `Retailer with id ${params} was deleted successfully`});
            });
        }
    });
});
...
app.delete("/product/:id", (req, res) => {
    ...
    db.get(selectSql, [params], (err, row) => {
        if (err) {
            ...
            res.status(404).send({error: `Product with id ${params} does not exists`});
        } else {
            db.run(deleteSql, [params], (err) => {
                if (err) {
                ...
                res.status(200).send({message: `Product with id ${params} was deleted successfully`});
            });
        }
    });
});
...
app.delete("/company/:id", (req, res) => {
    ...
    db.get(selectSql, [params], (err, row) => {
        if (err) {
            ...
            res.status(404).send({error: `Company with id ${params} does not exists`});
        } else {
            db.run(deleteSql, [params], (err) => {
                if (err) {
                ...
                res.status(200).send({message: `Company with id ${params} was deleted successfully`});
            });
        }
    });
});
